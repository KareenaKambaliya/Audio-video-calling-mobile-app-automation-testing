"""
Test Suite: Schedule Call Joining (Sheet: ScheduleCallJoining_Test_Cases)
Data-Driven Suite for Joining Scheduled sessions under different timing and permission states.
"""

import time
import pytest
from pages.schedule_page import SchedulePage
from pages.call_page import CallPage
from pages.login_page import LoginPage
from utils.excel_reader import get_sheet_test_cases
from utils.excel_reporter import ExcelReporter
from utils.logger import logger
from config import config

SHEET_NAME = "ScheduleCallJoining_Test_Cases"
TEST_CASES = get_sheet_test_cases(SHEET_NAME)


@pytest.mark.parametrize("test_case", TEST_CASES, ids=[tc.get("TC_ID", f"TC_{i}") for i, tc in enumerate(TEST_CASES)])
class TestScheduleJoiningSuite:

    def test_schedule_joining_scenario(self, driver, test_case):
        """Execute scheduled session joining scenario."""
        tc_id = test_case.get("TC_ID", "TC_SCJ")
        desc = test_case.get("Scenario_Description", "Schedule Joining")
        timing = test_case.get("Timing", "On time")
        expected_result = test_case.get("Expected_Result", "SUCCESS").upper()
        expected_msg = test_case.get("Expected_Message", "")

        # Check Execute column from Excel
        execute_flag = str(test_case.get("Execute", "Y")).strip().upper()
        if execute_flag != "Y":
            logger.info(f"Skipping {tc_id} because Execute flag is '{execute_flag}' in Excel.")
            pytest.skip(f"Execute flag is '{execute_flag}' in Excel")


        test_title = f"{tc_id} - {desc}"
        logger.info(f"\n{'=' * 60}\nRunning [{SHEET_NAME}]: {test_title}\n{'=' * 60}")

        login_page = LoginPage(driver)
        sched_page = SchedulePage(driver)
        call_page = CallPage(driver)

        # 1. Ensure logged in
        login_page.dismiss_initial_dialogs()
        if not login_page.is_already_logged_in():
            login_page.login(config.LOGIN_EMAIL, config.LOGIN_PASSWORD)
            time.sleep(3)

        # 2. Attempt joining scheduled call
        sched_page.join_scheduled_call()
        time.sleep(4)

        # 3. Assert Expected Result
        try:
            if expected_result == "SUCCESS":
                is_joined = call_page.is_call_active() or "joining" in driver.current_activity.lower() or "callactivity" in driver.current_activity.lower()
                screenshot = sched_page.take_screenshot(prefix=f"PASS_{tc_id}")
                assert is_joined, f"Expected to join call successfully. Msg: {expected_msg}"
                ExcelReporter.log_result(
                    test_name=test_title,
                    status="PASSED",
                    screenshot_path=screenshot,
                    error_message=f"Joined scheduled call successfully ({expected_msg})"
                )
            else:
                err = sched_page.get_error_message()
                screenshot = sched_page.take_screenshot(prefix=f"NEG_{tc_id}")
                is_blocked = not call_page.is_call_active() or bool(err)
                assert is_blocked, f"Expected join rejection, but joined! Error: {err}"
                ExcelReporter.log_result(
                    test_name=test_title,
                    status="PASSED",
                    screenshot_path=screenshot,
                    error_message=f"Join blocked as expected: '{err}' ({expected_msg})"
                )
        finally:
            if call_page.is_call_active():
                call_page.end_call()
